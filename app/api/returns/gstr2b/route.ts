...
/**
 * GET /api/returns/gstr2b
 * Fetch GSTR-2B data (final ITC statement)
 */
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("[v0] GSTR-2B API called:", { gstin, returnPeriod })
    ...
    const result = await makeEncryptedGstApiCall("/returns/gstr2b", payload, authToken, sek)
    ...
    return NextResponse.json({ success: false, error: result.error || "Failed to fetch GSTR-2B data" }, { status: 400 })
  } catch (error: any) {
    console.error("[v0] GSTR-2B API error:", error)
